...
public class URLStatePersistenceStrategyImpl implements URLStatePersistenceStrategy {
	...
	public URLStatePersistenceStrategyImpl(Request request, Logger logger, Environment environment,
	...
	public void updatePage(Link link) {
		PersistEnvironment persistEnvironment = environment.peek(PersistEnvironment.class);
		if ( persistEnvironment != null && persistEnvironment.include().size() == 0 ) {
			return;
		}
		//if ( environment.peek(GridEnvironment.class) != null || isEvent() ) {
			for ( Entry<String, String> entry : getMap().entrySet() ) {
				if ( persistEnvironment == null || persistEnvironment.include().contains(entry.getKey())) {
